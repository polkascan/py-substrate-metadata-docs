...
```python
call = substrate.compose_call(
    'CrabFeeMarket', 'update_locked_collateral', {'new_collateral': 'u128'}
)
```
...
| None | `LaneId` | ```[u8; 4]```
| None | `MessageNonce` | ```u64```
| None | `RewardItem<T::AccountId, BalanceOf<T, I>>` | ```{'to_assigned_relayers': 'scale_info::106', 'to_treasury': (None, 'u128'), 'to_message_relayer': (None, ('AccountId', 'u128')), 'to_confirm_relayer': (None, ('AccountId', 'u128'))}```
...
```python
result = substrate.query(
    'CrabFeeMarket', 'RelayersMap', ['AccountId']
)
```
