...
call = substrate.compose_call(
    'BridgeCrabParachainMessages', 'increase_message_fee', {
    'additional_fee': 'u128',
    'lane_id': '[u8; 4]',
    'nonce': 'u64',
}
...
```python
call = substrate.compose_call(
    'BridgeCrabParachainMessages', 'receive_messages_delivery_proof', {
    'proof': {
...
```python
call = substrate.compose_call(
    'BridgeCrabParachainMessages', 'receive_messages_proof', {
    'dispatch_weight': 'u64',
    'messages_count': 'u32',
    'proof': {
        'bridged_header_hash': '[u8; 32]',
...
```python
call = substrate.compose_call(
    'BridgeCrabParachainMessages', 'send_message', {
...
```python
call = substrate.compose_call(
    'BridgeCrabParachainMessages', 'set_operating_mode', {
    'operating_mode': (
        'Normal',
...
```python
call = substrate.compose_call(
    'BridgeCrabParachainMessages', 'set_owner', {'new_owner': (None, 'AccountId')}
)
```
...
```python
call = substrate.compose_call(
    'BridgeCrabParachainMessages', 'update_pallet_parameter', {
    'parameter': {
...
| Name | Type | Composition
| -------- | -------- | -------- |
| None | `T::Parameter` | ```{'CrabParachainToCrabConversionRate': 'u128'}```
...
result = substrate.query(
    'BridgeCrabParachainMessages', 'InboundLanes', ['[u8; 4]']
)
```
...
```python
result = substrate.query(
    'BridgeCrabParachainMessages', 'OutboundLanes', ['[u8; 4]']
)
```
...
```python
result = substrate.query(
    'BridgeCrabParachainMessages', 'OutboundMessages', [
    {
        'lane_id': '[u8; 4]',
        'nonce': 'u64',
    },
...
```python
result = substrate.query(
    'BridgeCrabParachainMessages', 'PalletOperatingMode', []
)
```
...
```python
result = substrate.query(
    'BridgeCrabParachainMessages', 'PalletOwner', []
)
```
...
#### Python
```python
constant = substrate.get_constant('BridgeCrabParachainMessages', 'BridgedChainId')
```
---------
## Errors
