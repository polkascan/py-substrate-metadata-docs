...
```python
call = substrate.compose_call(
    'XCMApp', 'try_claim_bridge_asset', {'nonce': 'u128'}
)
```
...
| -------- | -------- | -------- |
| None | `ParachainAppCall` | ```{'Transfer': {'asset_id': 'scale_info::11', 'sender': (None, {None: None, 'V2': {'parents': 'u8', 'interior': 'scale_info::86'}, 'V3': {'parents': 'u8', 'interior': 'scale_info::44'}}), 'recipient': 'AccountId', 'amount': 'u128'}, 'FinalizeAssetRegistration': {'asset_id': 'scale_info::11', 'asset_kind': ('Thischain', 'Sidechain')}, 'ReportXCMTransferResult': {'message_id': 'scale_info::11', 'transfer_status': ('Success', 'XCMTransferError')}}```
...
| -------- | -------- | -------- |
| None | `u128` | ```u128```
| None | `Option<H256>` | ```(None, 'scale_info::11')```
| None | `T::AccountId` | ```AccountId```
| None | `AssetId` | ```scale_info::11```
| None | `u128` | ```u128```
...
result = substrate.query(
    'XCMApp', 'AssetIdToMultilocation', ['scale_info::11']
)
```
...
```python
{
    ...
    'asset_id': 'scale_info::11',
    'is_refund': 'bool',
    'message_id': (None, 'scale_info::11'),
    'recipient': 'AccountId',
}
```
---------
### BridgeAssetTrapNonce
 Nonce for trapped message

#### Python
```python
result = substrate.query(
    'XCMApp', 'BridgeAssetTrapNonce', []
)
```
...
```
---------
### MultilocationToAssetId

#### Python
```python
result = substrate.query(
    'XCMApp', 'MultilocationToAssetId', [
    {
        'interior': {
            'Here': None,
            'X1': {
...
```python
result = substrate.query(
    'XCMApp', 'TrappedDoneResult', ['scale_info::11']
)
```
...
#### Python
```python
constant = substrate.get_constant('XCMApp', 'SelfLocation')
```
---------
### XorAssetId
#### Value
...
#### Python
```python
constant = substrate.get_constant('XCMApp', 'XorAssetId')
```
---------
## Errors
