...
| -------- | -------- | -------- |
| None | `ParachainAppCall` | ```{'Transfer': {'asset_id': '[u8; 32]', 'sender': (None, {None: None, 'V2': {'parents': 'u8', 'interior': 'scale_info::86'}, 'V3': {'parents': 'u8', 'interior': 'scale_info::44'}}), 'recipient': 'AccountId', 'amount': 'u128'}, 'FinalizeAssetRegistration': {'asset_id': '[u8; 32]', 'asset_kind': ('Thischain', 'Sidechain')}, 'ReportXCMTransferResult': {'message_id': '[u8; 32]', 'transfer_status': ('Success', 'XCMTransferError')}}```
...
result = substrate.query(
    'XCMApp', 'AssetIdToMultilocation', ['[u8; 32]']
)
```
...
```python
result = substrate.query(
    'XCMApp', 'TrappedDoneResult', ['[u8; 32]']
)
```
