...
call = substrate.compose_call(
    'Identity', 'add_registrar', {'account': '[u8; 20]'}
)
```
...
```python
call = substrate.compose_call(
    'Identity', 'kill_identity', {'target': '[u8; 20]'}
)
```
...
```python
call = substrate.compose_call(
    'Identity', 'set_account_id', {'index': 'u32', 'new': '[u8; 20]'}
)
```
...
| Name | Type | Composition
| -------- | -------- | -------- |
| sub | `T::AccountId` | ```[u8; 20]```
| main | `T::AccountId` | ```[u8; 20]```
| deposit | `BalanceOf<T>` | ```u128```
...
```python
{
    'deposit': 'u128',
    'info': {
        'additional': [('scale_info::146', 'scale_info::146')],
        'display': {
            'BlakeTwo256': 'h256',
            'Keccak256': 'h256',
            'None': None,
...
#### Return value
```python
[(None, {'account': '[u8; 20]', 'fee': 'u128', 'fields': 'u64'})]
```
---------
### SubsOf
